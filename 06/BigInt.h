...
typedef struct BigInt {
  unsigned char* digits;    // Array of digits 0-9.  Greater indices hold more
                            // significant digits.
  unsigned int num_digits;  // Number of digits actually in the number.
  unsigned int
      num_allocated_digits;  // digits array has space for this many digits
...
// comparison is of their absolute values.  Returns 1 if |a| > |b|, 0 if |a| ==
// |b|, and -1 if |a| < |b|.
int BigInt_compare_digits(const BigInt* a, const BigInt* b);

// Performs an unsigned addition of to_add to big_int; adds the digits without
// regard for the sign of either parameter.
void BigInt_add_digits(BigInt* big_int, const BigInt* to_add);

// Performs an unsigned subtraction of to_subtract from big_int; subtracts the
// digits without regard for the sign of either parameter.

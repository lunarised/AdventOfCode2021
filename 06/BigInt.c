...
BigInt* BigInt_construct(int value) {
  ...
  new_big_int->digits =
      malloc(new_big_int->num_allocated_digits * sizeof(unsigned char));
...
int BigInt_compare(const BigInt* a, const BigInt* b) {
  ...
  if (a->num_digits > 0 || a->digits[0] > 0 || b->num_digits > 0 ||
      b->digits[0] > 0) {
  ...
  return a->is_negative ? BigInt_compare_digits(b, a)
                        : BigInt_compare_digits(a, b);
}
...
int BigInt_compare_digits(const BigInt* a, const BigInt* b) {
  ...
  // Both have the same number of digits, so we actually have to loop through
  // until we find one that doesn't match.
  int i;
...
void BigInt_add(BigInt* big_int, const BigInt* addend) {
  if (big_int->is_negative == addend->is_negative) {
    ...
    // the digits result because changing those in big_int will affect the
...
void BigInt_subtract(BigInt* big_int, const BigInt* to_subtract) {
  ...
  // of the compare.
  unsigned int result_is_negative =
      BigInt_compare(big_int, to_subtract) > 0 ? 0 : 1;
...
void BigInt_subtract_digits(BigInt* big_int, const BigInt* to_subtract) {
  unsigned int digits_needed =
      MAX(big_int->num_digits, to_subtract->num_digits) + 1;
  BigInt_ensure_digits(big_int, digits_needed);
  ...
  for (i = 0; i < greater_int_num_digits; ++i) {
    ...
    if (i < smaller_int_num_digits) {
      new_digit =
          (int)greater_int_digits[i] - (int)smaller_int_digits[i] + carry;
    } else {
...
// Multiply using the pencil and paper method.  Complexity is O(n*m) where n, m
// are the number of digits in big_int and multiplier, respectively.
void BigInt_multiply(BigInt* big_int, const BigInt* multiplier) {
  ...
  // big_int to retain its original value throughout the course of the
  // calculation.
